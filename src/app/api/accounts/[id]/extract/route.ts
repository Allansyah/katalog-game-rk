...
export async function POST(
  ...
  try {
    ...
    if (!token || (token.role !== Role.RESELLER && token.role !== Role.SUPPLIER)) {
      return NextResponse.json({ error: 'Unauthorized - Reseller/Supplier only' }, { status: 401 });
    }
    ...
    const result = await db.$transaction(async (tx) => {
      // 1. Lock the account row and check availability
      ...
      const priceCalc = isOwner
        ...
        : calculateResellerPriceWithTier(basePrice, basePlatformFee, tierDiscountPercent);

      // 4. Check balance (skip for owner)
      if (!isOwner && buyer.balance < priceCalc.finalPrice) {
        throw new Error(`Insufficient balance. Need Rp ${priceCalc.finalPrice.toLocaleString()}, have Rp ${buyer.balance.toLocaleString()}`);
      }
      ...
      if (!isOwner) {
        ...
        // 7. Create pending balance for supplier (releases in 7 days)
        ...
        // 8. Update supplier's pending balance (for display)
        await tx.user.update({
        ...
        // 9. Create platform earning record (actual fee after tier discount)
      ...
      // 11. Update account status to SOLD
      await tx.account.update({
        where: { id: account.id },
      ...
      // For non-owner: update pending balance and platform earning with transaction ID
      if (!isOwner) {
        await tx.pendingBalanceRecord.updateMany({
          where: {
            userId: account.supplierId,
            transactionId: undefined as unknown as string,
          },
      ...
      return {
        ...
        transaction: {
          ...
          balanceAfter: isOwner ? buyer.balance : buyer.balance - priceCalc.finalPrice,
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to extract account';
